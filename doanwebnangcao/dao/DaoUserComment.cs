...
namespace doanwebnangcao.dao
{
    public class DaoUserComment
    {
        public static List<UserComment> getAllUserComment()
        {
            ...
            string strSQL = "SELECT MaComment, ID, FullName, TextComment, MaSanPham, Anh1 FROM UserComment";
        ...
        public static UserComment getOneUserComment(int _maComment)
        {
            UserComment objUC = null;
            //Lấy thông tin chuỗi kết nối từ Web.config
            string strConnection = ConfigurationManager.ConnectionStrings["ConnDB"].ConnectionString;
            //Viết câu lệnh truy vấn
            string strSQL = "SELECT MaComment, ID, FullName, TextComment, MaSanPham, Anh1 FROM UserComment where MaComment = '"+_maComment+"'";
            //Định nghĩa đối tượng Connection
            using (SqlConnection sqlConnection = new SqlConnection(strConnection))
            {
                //Khởi tạo đối tượng Command
        ...
        public static UserComment getOneUserCommentProduct(int _maComment)
        {
            UserComment objUC = null;
            //Lấy thông tin chuỗi kết nối từ Web.config
            string strConnection = ConfigurationManager.ConnectionStrings["ConnDB"].ConnectionString;
            //Viết câu lệnh truy vấn
            string strSQL = "SELECT UserComment.MaComment, UserComment.ID, FullName, TextComment, MaSanPham, Anh1 FROM UserComment where UserComment.MaComment = '" +_maComment+"' ";
            //Định nghĩa đối tượng Connection
            using (SqlConnection sqlConnection = new SqlConnection(strConnection))
            {
                //Khởi tạo đối tượng Command
